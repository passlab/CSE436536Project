...
void matvec_omp_task(int N, int tempN, REAL *A, REAL *B, REAL *C)  {
    ...
    if (N<=BASE) {
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < tempN; j++)
                temp += A[i * tempN + j] * B[j];
            C[i] = temp;
        }
    } else {
#pragma omp task shared(N,tempN,A,B,C)
        matvec_omp_task(N/2, tempN, A,B,C);
        matvec_omp_task(N-N/2, tempN, A+N/2, B+N/2, C+N/2);
#pragma omp taskwait
    }
